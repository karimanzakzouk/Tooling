...
def extractFemaleNames(filename,letter):
  ...
  for girlname in sorted_names:
    if girlname.startswith(letter):
      names.append(girlname + " " + names_rank[girlname])
  return names
...
def extractMaleNames(filename,letter):
  ...
  sorted_names = sorted(names_rank.keys(),reverse=True)
  for boyname in sorted_names:
    if boyname.startswith(letter):
      names.append(boyname + " " + names_rank[boyname])
  return names
...
def extractDocAndPopularityDate(filename):
  # +++your code here+++
  f=open(filename,'r')
  match = re.search(r'\w*\s\w*\,\s\w*',f.read())
  s1 = match.group()
  f.seek(0)
  match = re.search(r'\w*\s\w\w\s\d\d\d\d',f.read())
  s2=match.group()
  return str(s1+' and '+s2)
